...
@Component
@Repository("baseButtonsDao")
public class BaseButtonsDao extends HibernateDaoSupport implements BaseButtonsDaoFacade {
    @Resource
    ...
    public List<BaseButtons> getButtonsList(int page, int limit, String swhere, String sort, String type) {
        ...
        if("true".equals(type)){
           //不分页
          return   (List<BaseButtons>) this.getHibernateTemplate().findByCriteria(criteria);
        }else{
            //分页
           return  (List<BaseButtons>) this.getHibernateTemplate().findByCriteria(criteria,(page-1)*limit,limit);
        }
    ...
    public Boolean saveOrUpdate(BaseButtons buttons) {
        try {
            this.getHibernateTemplate().saveOrUpdate(buttons);
    ...
    public boolean deleteById(String id) {
        try {
            this.getHibernateTemplate().delete(new BaseButtons(id));
            this.getHibernateTemplate().flush();
            return true;
