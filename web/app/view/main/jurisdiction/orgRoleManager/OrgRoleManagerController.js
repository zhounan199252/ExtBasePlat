        ...
        if (checkedOrg.length == 0) {
            ...
            if (checkedRole.length == 0) {
                Ext.MessageBox.confirm('提醒', '确定要清空所选组织机构的角色吗？', function (btn) {
                    if (btn == 'yes') {
                        var data = { 'orgs': checkedOrg, 'roles': checkedRole };
                        Tools.Method.ExtAjaxRequestEncap(Tools.Method.getAPiRootPath()+'/baseOrgInfo/orgRoleSave.do', 'POST', data, true, function (jsonData) {
                            if (jsonData) {
