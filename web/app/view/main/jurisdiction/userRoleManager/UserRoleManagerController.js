        ...
        if (checkedUser.length == 0) {
            ...
            if (checkedRole.length == 0) {
                Ext.MessageBox.confirm('提醒', '确定要清空所选用户的角色吗？', function (btn) {
                    if (btn == 'yes') {
                        var data = {'users': checkedUser, 'roles': checkedRole  };
                        Tools.Method.ExtAjaxRequestEncap(Tools.Method.getAPiRootPath()+'/baseUserInfo/saveUserRoleRelation.do', 'POST', data, true, function (jsonData) {
                            if (jsonData) {
                                //重新加载grid
